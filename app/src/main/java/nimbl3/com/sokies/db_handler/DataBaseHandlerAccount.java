...
public class DataBaseHandlerAccount extends SQLiteOpenHelper {
    ...
    final static String CUSTOMER_ALERT = "alert";
    final static String CUSTOMER_PROJECT = "project";
    final static String CUSTOMER_MVH_PROJECT = "mvh_project";
    final static String CUSTOMER_LEADS = "leads";
    final static String CUSTOMER_CAMPAINING = "campaining";
    final static String CUSTOMER_CONTACT = "contact";
    final static String CUSTOMER_ACTIVITIES = "activities";
    final static String CUSTOMER_TRANSACTION = "transaction";
    final static String CUSTOMER_CATEGORY_TYPE = "customer_category";
    ...
    final static String CREATE_ACCOUNT_NEW = "create table " + TABLE_NAME_ACCOUNT + "(" + CUSTOMER_ID + " integer primary key," + CUSTOMER_CATEGORY_TYPE + " text," + CUSTOMER_FIRST_NAME + " text,"+ CUSTOMER_EMAIL_ID + " text,"+ CUSTOMER_TELEPHONE + " text," + CUSTOMER_DETAILS + " text);";
    ...
    public Boolean insert_account_detail_new(AccountDataSet accountDataSets) {
        ...
        contentValues.put(CUSTOMER_CATEGORY_TYPE, accountDataSets.getMcategoryType());
    ...
    public String get_customer_alert_permission(){
        String query = SELECT_VALUE_SELECT + CUSTOMER_ALERT + " " + SELECT_VALUE_FROM + TABLE_NAME_ACCOUNT ;
        SQLiteDatabase db = this.getReadableDatabase();
        cursor = db.rawQuery(query, null);
        if (cursor != null) {
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                String result = cursor.getString(cursor.getColumnIndex(CUSTOMER_ALERT));
                if (result!=null) {
                    return result;
                }
            }
    ...
    public String get_customer_telephone(){
    ...
    public String get_customer_Category_Type(){
        String query = SELECT_VALUE_SELECT + CUSTOMER_CATEGORY_TYPE + " " + SELECT_VALUE_FROM + TABLE_NAME_ACCOUNT ;
        SQLiteDatabase db = this.getReadableDatabase();
        cursor = db.rawQuery(query, null);
        if (cursor != null) {
            for (cursor.moveToFirst(); !cursor.isAfterLast(); cursor.moveToNext()) {
                String result = cursor.getString(cursor.getColumnIndex(CUSTOMER_CATEGORY_TYPE));
                if (result!=null) {
                    return result;
                }
            }
